...
void Unsigned::padShorterNumber(const Unsigned*& pA, const Unsigned*& pB,
                                Unsigned& padded) {
...
bool Unsigned::isZero() const {
  ...
  for (digits_t::const_iterator it = mDigits.begin(); it != mDigits.end();
       it++) {
...
int Unsigned::charToInt(const char c) { return c - '0'; }
...
void Unsigned::clear() { mDigits.erase(mDigits.begin(), mDigits.end()); }

void Unsigned::mkError() {
  clear();  // Doesn't really set an error.  But will case unit tests to fail
...
int Unsigned::toInt() const { return atoi(toString().c_str()); }

long Unsigned::toLong() const { return atol(toString().c_str()); }

void Unsigned::set(const Unsigned& in) { mDigits = in.mDigits; }

Unsigned::Unsigned(const Unsigned& in) { set(in); }

Unsigned::Unsigned(const char* in) { saveNumber(in); }
...
Unsigned Unsigned::add(const Unsigned& a, const Unsigned& b) {
  ...
  int carry = 0;
  for (int i = workingA.length() - 1; i >= 0; i--) {
...
Unsigned Unsigned::multiply(const Unsigned& a, const Unsigned& b) {
  ...
  for (size_t i = 0; i < b.length(); i++) {
    ...
    size_t j = 0;
    for (; j < a.length(); j++) {
      const int aIndex = a.length() - 1 - j;
      const int aDigit = a.mDigits[aIndex];
      const int digit = bDigit * aDigit + carry;
      const int ppIndex = partProduct.length() - 1 - j - i;
...
UnsignedDivide Unsigned::divideByTwoWithRem(const Unsigned& dividend) {
  ...
  const int len1 = working.length() - 1;
  int first, second;
...
UnsignedDivide Unsigned::divideWithRemSlow(const Unsigned& dividend,
...
Unsigned Unsigned::findDivisibleSizedChunk(const Unsigned& runningDividend,
...
UnsignedDivide Unsigned::divideWithRemFast(const Unsigned& dividend,
                                           const Unsigned& divisor) {
  ...
  bool fits;
  for (;;) {
    const Unsigned chunk = findDivisibleSizedChunk(runningDividend, dividend,
                                                   divisor, iDividendPos, fits);
    if (!fits) {
    ...
    runningDividend.appendRight(UNASSIGNED,
                                dividend.length() - iDividendPos - 1);
...
void Unsigned::divide(const Unsigned& other) { set(divide(*this, other)); }
...
ArbNum::ArbNum(const ArbNum& in) { set(in); }

ArbNum::ArbNum(const char* s) { saveNumber(s); }
...
ArbNum::ArbNum(const bool b) { set((int)b); }
...
ArbNumDivide ArbNum::divideWithRem(const ArbNum& dividend,
                                   const ArbNum& divisor) {
  ArbNumDivide result;
  ...
  // Both are positive
  const UnsignedDivide udivide =
      Unsigned::divideWithRemFast(dividend.mUnsigned, divisor.mUnsigned);
...
ArbNum ArbNum::random(const ArbNum& n) { return Unsigned::random(n.mUnsigned); }
...
void ArbNum::add(const ArbNum& other) { set(add(*this, other)); }

void ArbNum::subtract(const ArbNum& other) { set(subtract(*this, other)); }

void ArbNum::multiply(const ArbNum& other) { set(multiply(*this, other)); }

void ArbNum::divide(const ArbNum& other) { set(divide(*this, other)); }

void ArbNum::mod(const ArbNum& other) { set(mod(*this, other)); }

void ArbNum::pow(const ArbNum& other) { set(pow(*this, other)); }
...
bool ArbNum::testPow(const long a, const long b) {
  ...
  const long lCorrect = (long)::pow(a, b);
  if (lResult != lCorrect) {
...
bool ArbNum::testAll() {
  ...
  const long checkLong[] = {LONG_MIN, INT_MIN, SHRT_MIN, 0,
